...
public class StreamTest
{
    ...
    public void testStreamNull() throws Exception
    {
        ...
        Map<String, Integer> collect = studentList.stream()
                                                  .collect(Collectors.groupingBy(Student::getName, Collectors.summingInt(Student::getNum)));
    ...
    public void sortRank() throws Exception
    {
        ...
        for (int i = 0; i < 50; i++)
        {
            indicatorIndexScoreVOList.add(new IndicatorIndexScoreVO(city+i,new BigDecimal(random.nextInt(100)),yearMonth1));
        }
        for (int i = 0; i < 50; i++)
        {
            indicatorIndexScoreVOList.add(new IndicatorIndexScoreVO(city+i,new BigDecimal(random.nextInt(100)),yearMonth2));
        }
        for (int i = 0; i < 50; i++)
        {
            indicatorIndexScoreVOList.add(new IndicatorIndexScoreVO(city+i,new BigDecimal(random.nextInt(100)),yearMonth3));
        }
        //-----------数据准备完毕
        Map<YearMonth, List<IndicatorIndexScoreVO>> map = indicatorIndexScoreVOList.stream()
                                                                                       .collect(Collectors.groupingBy(IndicatorIndexScoreVO::getDataTime));
        ...
        for (Map.Entry entry : entrySet)
        {
            ...
            List<IndicatorIndexScoreVO> sortedList = listByDate.stream()
                                                            .sorted(Comparator.comparing(IndicatorIndexScoreVO::getIndexScore).reversed())
